...
Soit \(G=(V,E)\) un graphe biparti. Pour construire un couplage maximum de \(G\), on peut améliorer tant que possible un couplage initialement vide en trouvant et utilisant des chemins d'augmentation.
...
\begin{proof}\leavevmode
  \begin{itemize}
    \item L'entier \(|C|\) est incrémenté à chaque itération de la boucle principale. Or, \(|C| \leq |E|\), donc l'algorithme termine.
